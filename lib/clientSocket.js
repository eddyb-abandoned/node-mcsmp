...
clientSocket.prototype.connect = function connect(server, username, password, protoVersion, sessionID) {
    if(password !== undefined) {
        auth.login(username, password, function(error, username, sessionID) {
            if(error)
        ...
        while(start < data.length) {
            ...
            if(packet.type == proto.packets.KEEPALIVE)
                ...
                if(packet.hash == '-')
                    ...
                    auth.joinServer(this.username, this.sessionID, packet.hash, function(error) {
                        if(error)
                            this.emit('error', error);
                        else
                            this.write.bind(this, proto.packets.LOGIN, {protoVersion: this.protoVersion, username: this.username});
                    }.bind(this));
                }
